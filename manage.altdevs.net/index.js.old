...
function setPart(newKeys) {
	...
	doAjax("save", programming);
	ddSet("none", "");
}

function template(pstring) {
	programming = pstring;
	fetchPage('Configuration');
	ajaxRetFn = '$("#templateLoaded").show(500);doAjax("save", programming);';
}
...
function stopPing() {
	ws.onmessage = function(msg) { return; };
}
...
function retr(cmd, data) {
	...
	$.post( "/index.php", txObj)
		.done(function(rxObj) {
			...
			} else if (cmd == "save" && $("#ddProfile").html() != "Profile") {

				doAjax("saveProfile", { name: $("#ddProfile").html(), data: programming });
...
function populateLastCmds() {
	...
	for (var c in lastCmds) {
		...
		s = "<p>" + locMapImg[ lastCmds[c][0] + lastCmds[c][1] ] + " " + (m ? m : friendlyKeys(lastCmds[c].substr(2)) ) + "</p>" + s;
	}
...
function popClicks() {
	/* Generate keymaps */
	var ol = $(".img-fluid")[0].offsetLeft;
	var ot = $(".img-fluid")[0].offsetTop;
	var sf = $(".img-fluid")[0].width / 650;
...
function popPage(p) {
	switch (p) {
		...
		case "Advanced":
			var r = decodeProg();
			for (var k in r) $("#" + k).val(r[k]);
			$("#enableAdjust label").removeClass("active");
			$("#enableAdjust input[value=" + r['enableAdjust'] + "]").parent().addClass("active");
			$("input").on("change", function(e) {
				//r[ e.target.id ] = e.target.val;dd
				doAjax("set", encodeId(e.target.name) + "," + e.target.value);
			});
			break;
...
function profileAdd() {
	...
	$("#profileSel").append('<a class="dropdown-item" href="javascript:void(0)" onclick="doAjax(\'getProfile\', \'' + $("#profName").val() + '\');">' + $("#profName").val() + '</a>');
